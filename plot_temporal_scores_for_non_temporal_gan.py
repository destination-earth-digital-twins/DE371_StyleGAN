...
if __name__=="__main__" :
    ...
    diurnal_cycle = np.zeros((2, len(pixel_coordinate_dict), 2, params.nb_timesteps-2, nb_sample_total))
        ...
        for key_id, key in enumerate(pixel_coordinate_dict):
            ...
            for member_id, Ens_r_member in enumerate(Ens_r):
                ...
                for t in range(0, params.nb_timesteps-2):
                    ...
                    u = gen_sample[t+1][0][pixel_coordinate[0]][pixel_coordinate[1]]
                    v = gen_sample[t+1][1][pixel_coordinate[0]][pixel_coordinate[1]]
                    t2m = gen_sample[t+1][2][pixel_coordinate[0]][pixel_coordinate[1]]
    ...
    for key_id, key in enumerate(pixel_coordinate_dict):
        ...
        ax[0].plot(range(len(diurnal_cycle[0,key_id,0])),  diurnal_cycle[0,key_id,0], linewidth=6, color='k', label='AROME')
        ax[0].plot(range(len(diurnal_cycle[1,key_id,0])),  diurnal_cycle[1,key_id,0], linewidth=6, label='Generated')
        ax[0].set_ylabel('Wind speed (m/s)', size = 30)
        ax[0].set_xticks(range(len(list_ticks)), labels=list_ticks)

        ax[1].plot(range(len(diurnal_cycle[0,key_id,1])), diurnal_cycle[0,key_id,1], linewidth=6, color='k', label='AROME')
        ax[1].plot(range(len(diurnal_cycle[1,key_id,1])),  diurnal_cycle[1,key_id,1], linewidth=6, label='Generated')
    ...
    ax[0].plot(range(len(pearsons_first_to_each_leadtime_img[0, 0])),  pearsons_first_to_each_leadtime_img[0, 0], linewidth=6, color='k', label='AROME')
    ax[0].plot(range(len(pearsons_first_to_each_leadtime_img[1, 0])),  pearsons_first_to_each_leadtime_img[1, 0], linewidth=6, label='Generated')
    ax[0].set_ylabel('Wind speed U (m/s)', size = 30)
    ax[0].set_xticks(range(len(list_ticks)), labels=list_ticks)
    
    ax[1].plot(range(len(pearsons_first_to_each_leadtime_img[0, 1])),  pearsons_first_to_each_leadtime_img[0, 1], linewidth=6, color='k', label='AROME')
    ax[1].plot(range(len(pearsons_first_to_each_leadtime_img[1, 1])),  pearsons_first_to_each_leadtime_img[1, 1], linewidth=6, label='Generated')
    ax[1].set_ylabel('Wind speed V (m/s)', size = 30)
    ax[1].set_xticks(range(len(list_ticks)), labels=list_ticks)

    ax[2].plot(range(len(pearsons_first_to_each_leadtime_img[0, 2])), pearsons_first_to_each_leadtime_img[0, 2], linewidth=6, color='k', label='AROME')
    ax[2].plot(range(len(pearsons_first_to_each_leadtime_img[1, 2])), pearsons_first_to_each_leadtime_img[1, 2], linewidth=6, label='Generated')
    ...
    fig, ax = plt.subplots(nrows=3, ncols=1, figsize=(16,16))
    ax[0].plot(range(len(pearsons_sliding_img[0, 0])),  pearsons_sliding_img[0, 0], linewidth=6, color='k', label='AROME')
    ax[0].plot(range(len(pearsons_sliding_img[1, 0])),  pearsons_sliding_img[1, 0], linewidth=6, label='Generated')
    ax[0].set_ylabel('Wind speed U (m/s)', size = 30)
    ax[0].set_xticks(range(len(list_ticks)), labels=list_ticks)

    ax[1].plot(range(len(pearsons_sliding_img[0, 1])),  pearsons_sliding_img[0, 1], linewidth=6, color='k', label='AROME')
    ax[1].plot(range(len(pearsons_sliding_img[1, 1])),  pearsons_sliding_img[1, 1], linewidth=6, label='Generated')
    ax[1].set_ylabel('Wind speed V (m/s)', size = 30)
    ax[1].set_xticks(range(len(list_ticks)), labels=list_ticks)
    
    ax[2].plot(range(len(pearsons_sliding_img[0, 2])), pearsons_sliding_img[0, 2], linewidth=6, color='k', label='AROME')
    ax[2].plot(range(len(pearsons_sliding_img[1, 2])), pearsons_sliding_img[1, 2], linewidth=6, label='Generated')
    ax[2].set_ylabel('Temperature at 2m (K)', size = 30)
    ax[2].set_xticks(range(len(list_ticks)), labels=list_ticks)
    ...
    ax[1].plot(range(len(temporal_difference[0, 1])),  temporal_difference[0, 1], linewidth=6, color='k', label='AROME')
    ax[1].plot(range(len(temporal_difference[1, 1])),  temporal_difference[1, 1], linewidth=6, label='Generated')
    ax[1].set_ylabel('Wind speed V (m/s)', size = 30)
    ax[1].set_xticks(range(len(list_ticks)), labels=list_ticks)

    ax[2].plot(range(len(temporal_difference[0, 2])), temporal_difference[0, 2], linewidth=6, color='k', label='AROME')
    ax[2].plot(range(len(temporal_difference[1, 2])),  temporal_difference[1, 2], linewidth=6, label='Generated')
    ax[2].set_ylabel('Temperature at 2m (K)', size = 30)
    ax[2].set_xticks(range(len(list_ticks)), labels=list_ticks)
