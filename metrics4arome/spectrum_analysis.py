...
import metrics4arome.wind_comp as wc
...
def PSD_compare_multidates(obsdata, fake_data, real_data, debiasing=False):

    channels = real_data.shape[2]
    H,W = real_data.shape[-2], real_data.shape[-1]

    real_data = deepcopy(real_data)
    fake_data = deepcopy(fake_data)
    ...
    res = PSD_compare(resh_real, resh_fake)    
