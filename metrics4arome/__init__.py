...
class metric2D():
    ...
    def __call__(self, *args, **kwargs):
        ...
        if len(data) == 3:
            ...
            return self.func(data[0], data[1], data[2],**reliq_kwargs) 
        else :
...
distance_metrics = {'Orography_RMSE', 'W1_Center', "W1_Center_NUMPY", "W1_random",
                    "W1_random_NUMPY",
                    "pw_W1", "spectral_dist", "spectral_dist_all", "ls_dist",
                    "SWD_metric", "SWD_metric_all", "SWD_metric_torch", "SWD_metric_torch_all",
                    "fid","scat_SWD_metric", "scat_rmse",
...
spectral_dist_all = metric2D('Power Spectral Density RMSE  ',\
                  Spectral.PSD_compare, vars_wo_orog, names = ['PSDu', 'PSDv','PSDt2m'])
...
spec_com = metric2D('PSD multi-dates comparison', 
                    Spectral.PSD_compare_multidates, 
                    vars_wo_orog, names = ['PSDff', 'PSDdd', 'PSDt2m'])
...
ls_dist = metric2D('Correlation length RMSE', lambda real, fake : ls.length_scale_abs(real, fake, sca =scale),\
                   vars_wo_orog, names = ['Lcorr_u', 'Lcorr_v', 'Lcorr_t2m'])
