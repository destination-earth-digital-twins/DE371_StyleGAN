...
Importance Sampling helps avoid over-representing samples with little or no precipitation in the dataset. By assigning an "importance" score to each sample based on its contribution to the training process, we can prioritize the most significant data points. 
...
Using the `method_type=merge_into_giga_file` parameter in `main.py`, all samples are merged into a single large file. This process outputs:
...
- Filters out less significant samples, retaining only those deemed important for training.
...
- This method performs **n random samplings** on the entire dataset.
- It generates **n CSV files**, each containing different subsets of the original dataset.

### 4. Select Ensemble Members

After importance sampling and optional bootstrap:
- We ensure that for a given lead time and date, all ensemble members are included in the dataset.
- If 8 members of an ensemble are found in the current CSV file, the method selects all 16 members from the original dataset.
