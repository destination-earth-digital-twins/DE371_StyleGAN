...
def get_expe_parameters():
    ...
    parser.add_argument('--output_dir', type=str, default='/project/home/p200177/DE_371/experiments_WP1/gan_training/exp2/')
...
def prepare_expe(config):
    ...
    +str(config.batch_size)+'_'+str(config.lr_D)+'_'+str(config.lr_G)+'_ch-mul_'+str(config.channel_multiplier)+\
        '_vars_'+'_'.join(str(var_name) for var_name in config.var_names)+f'_noise_{config.use_noise}'
...
if __name__=="__main__":
    ...
    slurm_dir = f"{home_dir}/gan/slurms/"
    config_directory = f"{home_dir}/gan/configs/"
    ...
    for i, params in enumerate(expe_list):
        ...
        if dirs.platform=='priam':

            try:
                assert os.uname().nodename=='priam-sidev'

            except AssertionError:
                raise ValueError(f"You specified platform priam but nodename is {os.uname().nodename}")
            ...
            try:
                ...
                raise ValueError(f"You specified platform belenos but nodename is {os.uname().nodename}")
