...
from restyle_encoder.utils.common import numpyfy
...
def run():
    ...
    transforms_dict = dataset_args['transforms'](config, path = config.data_path+'/').get_transforms()
    ...
    for i in range(input_batch.shape[0]):
        results = [numpyfy(result_batch[i][iter_idx]) for iter_idx in range(config.n_iters_per_batch)]
    ...
    np.save(os.path.join(test_config.exp_dir, 'latents.npy'), all_latents)
