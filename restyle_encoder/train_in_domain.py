...
from options.inDomain_train_options import inDomainTrainOptions, createNamesFromLosses
from restyle_encoder.training.coach_in_domain import Coach


def main(namesFromLosses=False):
    
    config = inDomainTrainOptions().parse()
    
    if namesFromLosses : 
        config.exp_dir = config.exp_dir  + 'in_Domain_training/' + createNamesFromLosses(config)
