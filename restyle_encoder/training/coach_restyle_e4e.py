...
from restyle_encoder.models.e4e_modules.latent_codes_pool import LatentCodesPool
from restyle_encoder.models.e4e_modules.discriminator import LatentCodesDiscriminator
from restyle_encoder.models.encoders.restyle_e4e_encoders import ProgressiveStage
...
class Coach:
	def __init__(self, config, prev_train_checkpoint=None):
		...
		if self.config.dataset_type == "cars_encode":
		...
		# Initialize logger
		log_dir = os.path.join(config.exp_dir, 'logs')
	...
	def configure_datasets(self):
		...
		transforms_dict = dataset_args['transforms'](self.config).get_transforms()
