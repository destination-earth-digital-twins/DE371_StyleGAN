...
from restyle_encoder.models.encoders import restyle_psp_encoders
...
def load_and_initialize_encoder(ckpt, config):
    
    encoder = restyle_psp_encoders.ResNetBackboneEncoder(n_styles, config)
...
if __name__=="__main__" :    
    ...
    config = Namespace(**{'input_nc' : 6})
    
    g_ema = load_and_initialize_stylegan('285000.pt').to(device)
    
    encoder = load_and_initialize_encoder('best_model.pt', config).to(device)
