...
if __name__=="__main__" :
   ...
   #SANS EP parser.add_argument('--ckpt_dir', type = str, 
    ...
    parser.add_argument('--output_dir',type = str, 
                        default ='/home/mrmn/bonamya/de371_stylegan/results_EP/inversion_batch/')
    # Pack Directory - PATH where the packed ensembles will be saved
    parser.add_argument("--pack_dir", type=str, 
                        default = '/home/mrmn/bonamya/de371_stylegan/results_EP/pack_batch/') # storing "packed" (normalized) real data
    ...
    classes = pickle.load(open('/home/mrmn/bonamya/de371_stylegan/tests_inv_batch/class_samples.p','rb'))
    ...
    for scenario in scenarii:
        ...
        for batch_idx, batch in enumerate(samples[:-1]):
            ...
            if not os.path.exists(params.output_dir +f'w_{scenario}_{batch_idx}_1000.npy'): #checking for already teer
                ...
                batch[:,2] = -1.0 + 2 * (batch[:,2] - Mins[2]) / (Maxs[2] - Mins[2]) 
                batch[:,3] = -1.0 + 2 * (batch[:,3] - Mins[3]) / (Maxs[3] - Mins[3]) 
