...
if __name__=="__main__" :
    ...
    parser.add_argument('--w_inversion_dir', type=str, default='')
    parser.add_argument('--w_samples_dir',   type=str, default='') # samples generated with mkl_w_sample.py
    parser.add_argument('--inversion_step',  type=int, default=1000)
    parser.add_argument('--case',            type=str, default="", help="specific inversion case to consider") # %Y-%m-%d_lt
    ...
    w_inv = np.load(f"{args.w_inversion_dir}/w_{args.case}_{args.inversion_step}.npy") # (16,14,512)
    ...
    member_id = 0
    w=w_inv
    title=f'inv_{args.case}'
    around_latent_mean = False
    for first_pca_axis in range(0,5):
        for second_pca_axis_id in range(first_pca_axis, 5):
            if first_pca_axis != second_pca_axis_id:
                ...
                # Normalizing w
                if around_latent_mean :
                    w_member = np.mean(w, axis=0) #(w[member_id]-np.mean(w[member_id], axis=0))
                else : 
                    w_member = w[member_id]-np.mean(w, axis=0)
                # Forwarding w to obtain initial unperturbated generated sample
                img_from_w_member = G([torch.from_numpy(w_member).unsqueeze(0).cuda()], input_is_latent=True)[0].cpu().detach().numpy()[0]
                variables = ['u', 'v', 't2m']
                ...
                for var_id, var in enumerate(variables):
                    ...
                    for id_ax1, perturbation_intensity_pca1 in enumerate(np.linspace(-3,3,7)) :
                        for id_ax2, perturbation_intensity_pca2 in enumerate(np.linspace(-3,3,7)) :
                            # Perturbation vector 
                    ...
                    if diff_flag :
                        ...
                        fig.savefig(args.output_dir+f'perturbation_of_{var}_{style_vector}_pca_axis_{(first_pca_axis,second_pca_axis_id)}_{title}_{around_latent_mean}_sample.png')
                    plt.close() 