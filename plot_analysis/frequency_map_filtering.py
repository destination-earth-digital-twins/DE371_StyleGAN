...
if __name__=="__main__" :
    ...
    parser.add_argument('--output_dir',type = str, 
                        default ='/project/scratch/p200177/DE_371/victorsanchez/results/frequency_map/low_pass_filtering/')
    ...
    for date_ in list_dates:
        ...
        for lt in params.leadtimes:
            ...
            fig, ax = plt.subplots(figsize=(18,18), nrows=5, ncols=3)
            fft2_x = np.array([fftpack.fftshift(fftpack.fft2((x[i].numpy())).astype(np.float32)) for i in range(3)])
            ...
            # low pass filter
            n = 50
            fft2_x_low_passed = deepcopy(fft2_x)
            for i in range(w):
                for j in range(h):
                    if (i < half_w-n or i > half_w+n) or (j < half_h-n or j > half_h+n): 
                        fft2_x_low_passed[:,i,j] = fft2_x[:,i,j]*0

            freq_x_low_passed = [(20*np.log10(0.1+fft2_x_low_passed[i])).astype(np.int8) for i in range(3)]
            x_low_passed = np.array([fftpack.ifft2(fftpack.ifftshift(fft2_x_low_passed[i])).real for i in range(3)])  
            # x_low_passed = np.array([np.maximum(0, np.minimum(x_low_passed[i], 255)) for i in range(3)])  
            ...
            ax[1][0].imshow(freq_x[0], clim=(np.array(freq_x).min(), np.array(freq_x).max()))
            ax[1][1].imshow(freq_x[1], clim=(np.array(freq_x).min(), np.array(freq_x).max()))
            ax[1][2].imshow(freq_x[2], clim=(np.array(freq_x).min(), np.array(freq_x).max()))

            ax[2][0].imshow(freq_x_low_passed[0], clim=(np.array(freq_x).min(), np.array(freq_x).max()))
            ax[2][1].imshow(freq_x_low_passed[1], clim=(np.array(freq_x).min(), np.array(freq_x).max()))
            ax[2][2].imshow(freq_x_low_passed[2], clim=(np.array(freq_x).min(), np.array(freq_x).max()))

            ax[3][0].imshow(x_low_passed[0], origin='lower', clim=(x.min(), x.max()))
            ax[3][1].imshow(x_low_passed[1], origin='lower', clim=(x.min(), x.max()))
            ax[3][2].imshow(x_low_passed[2], origin='lower', clim=(x.min(), x.max()), cmap="coolwarm")

            ax[4][0].imshow(x_low_passed[0]-x[0].numpy(), origin='lower', cmap="RdYlGn")
            ax[4][1].imshow(x_low_passed[1]-x[1].numpy(), origin='lower', cmap="RdYlGn")
            ax[4][2].imshow(x_low_passed[2]-x[2].numpy(), origin='lower', cmap="RdYlGn")
            figname = params.output_dir+f'low_passed_filter_Arome_{datename}_{lt}_{n}.png'
            fig.tight_layout()
