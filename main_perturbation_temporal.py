...
def compute_generate_save(G, params, metrics_list, Means, Mins, Maxs, apply_log_transform):
    ...
    for lt_id, lt in enumerate(params.leadtimes):
        ...
        if params.feature_insertion:
            Ens_feature=torch.tensor(np.load(params.data_dir + f'feature_{params.date_index}_{lt}_{params.inv_step}.npy').astype(np.float32))
        ...
        gen_next = None # Add function that generates it by reproducing the function in scale_tune_temporal
    ...
    for date_ in liste_dates:
        ...
        for lt_id, lt in enumerate(params.leadtimes):    
            ...
            if not params.import_perturbation :
                ...
                label = 'imported_pert'
            if os.path.isfile(params.output_dir + f'/samples/genFsemble_{params.date_index}_{lt}_{params.inv_step}_{params.N_conditioners}_{label}.npy'):
                already_exist.append(True)
            else :
                already_exist.append(False)
            if os.path.isfile(params.output_dir + f'/samples/w_pert_{params.date_index}_{lt}_{params.inv_step}_{params.N_conditioners}_{label}.npy'):
                already_exist.append(True)
            else :
                already_exist.append(False)
