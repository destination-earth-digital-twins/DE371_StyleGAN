...
library = {'swagan' : {'G' :  'Generator', 'D' : 'Discriminator'}}
...
class ToRGB(nn.Module):
    def __init__(self, in_channel, out_channel, style_dim, upsample=True, blur_kernel=[1, 3, 3, 1]):
        ...
        if upsample:
            self.iwt = InverseHaarTransform(out_channel)
            self.upsample = Upsample(blur_kernel)
            self.dwt = HaarTransform(out_channel)
...
class Generator(nn.Module):
    def __init__(
        ...
        for i in range(3, self.log_size + 1):
            ...
            self.to_rgbs.append(ToRGB(out_channel, nb_var, style_dim))

            in_channel = out_channel

        self.iwt = InverseHaarTransform(nb_var)
...
class FromRGB(nn.Module):
    def __init__(self, in_channels, out_channel, downsample=True, blur_kernel=[1, 3, 3, 1]):
        super().__init__()

        self.downsample = downsample

        if downsample:
            self.iwt = InverseHaarTransform(in_channels)
            self.downsample = Downsample(blur_kernel)
            self.dwt = HaarTransform(in_channels)

        self.conv = ConvLayer(in_channels * 4, out_channel, 3)
...
class Discriminator(nn.Module):
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], nb_var=3):
        super().__init__()
        ...
        for i in range(log_size, 2, -1):
            ...
            self.from_rgbs.append(FromRGB(nb_var,in_channel,downsample=i != log_size))
            self.convs.append(ConvBlock(in_channel, out_channel, blur_kernel))

            in_channel = out_channel

        self.from_rgbs.append(FromRGB(nb_var, channels[4]))
