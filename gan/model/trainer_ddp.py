...
def AllocScheduler(sched, optimizer, gamma, network, base_lr=0.1):
    if sched == "exp":
        ...
        if is_main_gpu(): print(f"{network} scheduler set to cyclic scheduler")
        return torch.optim.lr_scheduler.CyclicLR(optimizer, base_lr=base_lr, max_lr=4*base_lr, cycle_momentum=False)
    else:
...
class Trainer():
    ...
    def plot(self, Step, modelG, samples, samples_z):
        ...
        if not self.config.multi_timestep_mode:
            ...
            plotFunc.online_temporal_sample_plot(self.config, batch, Step)
    ...
    def save_samples(self, number, Step, modelG):
        ...
        for i in range(number):
            z = torch.empty(16, self.config.latent_dim).normal_().cuda()
    ...
    def fit_(self, modelG, modelD, modelG_ema=None):
        ...
        for epoch in range(self.config.epochs_num):
            ...
            for i, batch in loop:
                ...
                if is_main_gpu():
                    loop.set_postfix_str(f"loss_d : {loss_d['d'].item():.5f} | loss_g : {loss_g['g'].item():.5f} | step: {step:.2f}")
                    #print("up")
                ...
                if self.save_step > 0 and (Step % self.save_step == 0 or Step == self.config.total_steps - 1) and is_main_gpu():
                    self.save_samples(self.config.sample_num, Step, modelG_ema)
                    self.plot(Step, modelG_ema, samples, samples_z)
                    if self.config.pretrained_model > -1:  # we don't want to save the first step when using a pretrained
