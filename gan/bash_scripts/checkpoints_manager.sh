...
MAX_TIME = $($TIMEOUT - 60) #MAX_TIME=$(( $TIME_OUT + 60 )) 
...
timeout $MAX_TIME tail -f $SLURM_FILE | while read ;
	...
	if [[ (${#IS_STOPPED} -gt 0) ]] ;
	  then
	  	...
	  	bash /home/mrmn/gandonb/stylegan4arome/cleaner.sh >/dev/null 2>&1 ;
	  	echo "Launching new exp";
	  	SLURM_FILE="$(python3 /home/mrmn/gandonb/stylegan4arome/gan_horovod/expe_init.py --pretrained_model=$CKPT --SET_NUM=$EXPE_SET --max_relaunch=$(($MAX_COUNT - 1 )) | tail -1)" ;
