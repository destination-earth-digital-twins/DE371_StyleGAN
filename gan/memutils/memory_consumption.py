...
def log_mem_consumption(modelG, modelD, config, mem_g, mem_d, mem_opt, mem_cuda):
    ...
    df = pd.DataFrame(data=0, index=["Cuda_inst", "Generator", "Discriminator", "Optim", "Batch_max", "Total"],
    ...
    df.loc["Cuda_inst"] = [0, mem_cuda / 1024 ** 3, config.batch_size, config.crop_size[0],
                           len(config.var_names)]  # if not config.mean_pert else len(config.var_names)*2]
