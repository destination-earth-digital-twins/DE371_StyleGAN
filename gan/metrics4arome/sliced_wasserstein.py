...
class SWD_API:
    def __init__(self, image_shape, numpy=False):
        ...
        res = image_shape[1]
        while res >= 16:
            self.resolutions.append(res)# changer pour un nombre de fois ou on filtre
            res //= 2
...
class SWD_API2:
    def __init__(self, numpy=False, ch_per_ch = False):
        self.nhood_size         = 7
        self.nhoods_per_image   = 128
        self.dir_repeats        = 4
        self.dirs_per_repeat    = 128
        ...
        self.numpy=numpy
        filt_nb = 0
        self.ch_per_ch = ch_per_ch
        while filt_nb <= 3:
            self.resolutions.append(filt_nb)
            filt_nb += 1

    def get_metric_names(self):
        return ['SWDx1e3_%d_filt' % res for res in self.resolutions] + ['SWDx1e3_avg']
    ...
    def End2End(self, real, fakes):
        ...
        if self.ch_per_ch:
            for i in range(fakes.shape[1]): #compute SWD channel per channel
                self.begin('fakes')
        
                self.feed(fakes[:, i:i+1, :, :])
        
                self.feed(real[:, i:i+1, :, :])
