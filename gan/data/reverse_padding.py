...
def reflect(tableau):
    ...
    head = np.tile(inv_true_image, (0 if idx[0] == 0 else (idx[0] // n) + 1, 1))[-idx[0]:]
    new_array = np.concatenate((head, true_image, tail))
    return new_array


def pad_image_with_mask(image, mask, pad_value=9999):
    assert image.shape == mask.shape, "Les dimensions de l'image et du masque ne correspondent pas."
...
def process_chunk(input_folder, output_folder, pad, chunk):
    processed_files = 0
    for file_name in tqdm(chunk, desc="Traitement des fichiers", position=0, leave=True):
...
def main(input_folder, output_folder, pad):
    ...
    results = [pool.apply_async(process_chunk, (input_folder, output_folder, pad, chunk)) for chunk in chunks]
    pool.close()
    pool.join()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Applique reflect_padding aux fichiers _sampleXXXX.npy")
    parser.add_argument("input_folder", type=str, help="Dossier d'entrée contenant les fichiers _sampleXXXX.npy")
    parser.add_argument("output_folder", type=str, help="Dossier de sortie où enregistrer les images traitées")
