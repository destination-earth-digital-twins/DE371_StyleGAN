...
from normalize_funcs import MultiOptionNormalize
...
class ISDataset(Dataset):
    ...
    def __init__(self, config, dataset_handler_yaml, sample_method, variable_indices, transform):
        self.config = config
        self.dataset_handler_yaml = dataset_handler_yaml
        self.sample_method = sample_method
        self.VI = variable indices
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(self.dataset_handler_yaml["transform"]["log_transform_iteration"]):
            sample[0] = np.log(1 + sample[0])
        if self.dataset_handler_yaml["transform"]["symetrization"] and np.random.random() <= 0.5:
            sample[0] = -sample[0]
...
class ISData_Loader():
    ...
    def load_stat_files(self, normalization_type, str1, str2):
        ...
        mean_or_max_filename += "_log" * self.dataset_handler_yaml["transform"]["log_transform_iteration"]
        std_or_min_filename = f"{str2}_{self.dataset_handler_yaml['stat_version']}"
        std_or_min_filename += "_log" * self.dataset_handler_yaml["transform"]["log_transform_iteration"]
    ...
    def transform(self):
        ...
        if normalization != "None":
            if self.dataset_handler_yaml["transform"]["symetrization"]:
    ...
    def loader(self, world_size=None, local_rank=None, kwargs=None):
        ...
        self.sampler = DistributedSampler(dataset, num_replicas=hvd_size, rank=hvd_rank)
        if kwargs is not None:
