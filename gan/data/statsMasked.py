...
def normalizeDataset(data, path, var_indices = list(range(1,4)), verbose = False):
    ...
    Means = np.load(path + 'Mean_4_var.npy')[var_indices].reshape(len(var_indices),1,1)
    ...
    if verbose :
        ...
        funcs = {"Mean": lambda m: np.mean(m, axis = (0,-2,-1)), 
             "Min": lambda m: np.min(m, axis = (0,-2,-1)), 
             "Max" :lambda m: np.max(m, axis = (0,-2,-1))}
        
        print(computeUnderMask(normed,0.0, funcs["Mean"]), 
...
def collectAndCompute(base_path = base_path) :
    ...
    funcs = {"Mean": lambda m: np.mean(m, axis = (-2,-1)), 
    ...
    Sum = np.array([15390.530330379323, 62831.02876115003, 13717.80083381357, 18799140.00650119])
    
    gMin = np.array([0.0, -34.22730255126953, -35.41071319580078, 240.27041625976562])
    
    gMax = np.array([574.4132080078125, 40.038841247558594, 36.08406066894531, 321.4552001953125])
