...
class NormalizeUnderMasktorch(nn.Module):
    def __init__(self, mean, std, MaskV=9999.0, fill=0.0, scale=1.0, inplace=False) -> None:
        super().__init__()
    ...
    def forward(self, tensor: Tensor) -> Tensor:
        tensor = normalizeUnderMask(data=tensor.numpy(), MaskV=self.maskv, Mean=self.mean, 
                                    std=self.std, scale=self.scale, fill=self.fill)
        return ToTensor()(Tensor.transpose((1,2,0))).float()
...
class MultiOptionNormalize(object):
    def __init__(self, means, stds, maxs, mins, config, dataset_handler_yaml):
        ...
        if self.gaussian_std:
            ...
            self.gaussian_std_map = np.random.choice([-1, 1], size=self.config.crop_size) * self.gaussian_std
            self.gaussian_noise = np.mod(np.random.normal(0, self.gaussian_std, size=self.config.crop_size), self.gaussian_std_map)
            ...
            if np.ndim(self.stds) > 1:
                if self.dataset_handler_yaml["normalization"]["for_rr"]["blur_iteration"] > 0:
                    ...
                    for _ in range(self.dataset_handler_yaml["normalization"]["for_rr"]["blur_iteration"]):
                        self.stds[0] = scipy.ndimage.convolve(self.stds[0], gaussian_filter, mode='mirror')
            ...
            if np.ndim(self.maxs) > 1:
                if self.dataset_handler_yaml["normalization"]["for_rr"]["blur_iteration"] > 0:
                    gaussian_filter = np.float32([[1, 4,  6,  4,  1],
                                                ...
                                                [1, 4,  6,  4,  1]]) / 256.0
                    for _ in range(self.dataset_handler_yaml["normalization"]["for_rr"]["blur_iteration"]):
                        self.maxs[0] = scipy.ndimage.convolve(self.maxs[0], gaussian_filter, mode='mirror')
