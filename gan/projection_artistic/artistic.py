...
def get_boundaries(Zone) :
    ...
    """
    Zone_l=["NO","SO","SE","NE","C","SE_for_GAN", "SE_GAN_extend", "SE_for_GAN_terrestrial", "AROME_all"]
...
class canvasHolder():
    ...
    def project(self,padX=(5,15), padY=(5,5), ax=None) :
        ...
        Lat_max,Lon_max = grid_to_lat_lon(self.X_min+self.nb_lon+padX[1],
                                         self.Y_min+self.nb_lat+padY[1])
    ...
    def makeDomainBox(self, output_dir, padX=(300,30), padY=(180,180),full_domain=False):
        """
        plot the box of the studied subdomain on the AROME domain
        save figure on output dir
        """
        fig=plt.figure(figsize=(10,10))
        grid0=AxesGrid(fig,111, axes_class=self.axes_class, nrows_ncols=(1,1),
              axes_pad=0.3,
              cbar_pad=0.25,
              cbar_location="right",
              cbar_mode="none",
        ...
        lon_lat_rect=self.proj0.transform_point(recCoords[1], recCoords[0],
                                                ccrs.PlateCarree())
        
        lon_lat_ext=self.proj0.transform_point(recWidth, recHeight, ccrs.PlateCarree())
        

        ax.add_patch(patches.Rectangle((lon_lat_rect[0], lon_lat_rect[1]),\
        ...
        lon_lat_rect=self.proj0.transform_point(-8.18, arome_coords[0],
                                                ccrs.PlateCarree())
    ...
    def plot_data_normal(self,data, var_names,
                  plot_dir, pic_name,
                  contrast=False, cvalues=None, title=''):
            ...
            for i, var in enumerate(var_names):
                ...
                if not contrast:
                    ...
                    ims[Var+str(ind)]=axes[Var+str(ind)].pcolormesh(
                        ...
                        self.Coords[1],\
                        data_plot[i,:,:],\
                        cmap=cmap,alpha=1,vmin=Datamin[i]/2., vmax=Datamax[i]/2.,
        ...
        for ind in range(data.shape[0]):
            ...
            if not contrast :
                ...
                ims['wind_mag'+str(ind)]=axes['wind_mag'+str(ind)].pcolormesh(
                   ...
                   transform=self.proj_plot)
            axes['wind_mag'+str(ind)].add_feature(cfeature.COASTLINE.with_scale('10m')) # adding coastline
            axes['wind_mag'+str(ind)].add_feature(cfeature.BORDERS.with_scale('10m')) # adding borders
            ...
            if withQuiver:   
                ...
                ims['wind_dir'+str(ind)] = axes['wind_dir'+str(ind)].quiver(
                    ...
                    u=0.25*module[ind, ::quiSub, ::quiSub]*direction[0][ind,::quiSub,::quiSub],
                    v=0.25*module[ind, ::quiSub, ::quiSub]*direction[1][ind,::quiSub,::quiSub],
    ...
    def plot_abs_error(self,data, var_names,
                  ...
                  contrast=True, cmap_wind='viridis', cmap_t='Reds', suptitle=''):
            ...
            for i, var in enumerate(var_names):
                ...
                if not contrast:
                    ...
                    ims[Var+str(ind)]=axes[Var+str(ind)].pcolormesh(
                        ...
                        self.Coords[1],\
                        data_plot[i,:,:],\
                        cmap=cmap,alpha=1,vmin=data.min(axis=(0,2,3))[i], vmax=data.max(axis=(0,2,3))[i],
                        transform=self.proj_plot)
                ...
                if ind==0 or Var=='rr':
                    ...
                    grid.cbar_axes[i].colorbar(ims[Var+str(ind)]).set_label(label=Var + add_unit, size=40)#33
    ...
    def plot_abs_error_sev_cbar(self,data, var_names,
        ...
        #Title='3-fields states'
        fig.subplots_adjust(bottom=0.005, top=0.92 if len(var_names)>1 else 0.75, left=0.05, right=0.95)
