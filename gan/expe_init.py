...
if __name__=="__main__":
    ...
    ensemble['--config_dir'] = [f"{config_directory}Set_{dirs.SET_NUM}/"]
    ...
    for i, params in enumerate(expe_list):
        ...
        env_slurm = {**os.environ,
            ...
            "SLURM_SCRIPT": f"{home_dir}/{dirs.slurm_file}",
        ...
        print(f"env_slurm SLURM_SCRIPT: {env_slurm['SLURM_SCRIPT']}")
        print(f"env_slurm PYTHON_SCRIPT: {env_slurm['PYTHON_SCRIPT']}")
        
        sbatch_output = subprocess.run(['sbatch',slurm_dir + dirs.slurm_file, args], env=env_slurm, capture_output=False)
