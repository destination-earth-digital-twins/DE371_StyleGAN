...
Zones = {128:"SE_for_GAN", 256:"SE_GAN_extend", 512:"AROME_512", 1024:"AROME_all"}
...
def read_dataset_handler_config_file(config):
...
def compare_stats_metrics(set_path, steps=[[51000,102000,147000]], add_name='_16384_16384', lat_dims=[512], chms=[2], bss=[16],
    ...
    for i, metr in enumerate(metrics):
        if metr=='SWD_metric':
            ...
            ax.legend(framealpha = 0.8, fontsize=12)
    suptitle = f"Comparison of different distance metrics "
    suptitle += f"(domain = {dom_size[0]}x{dom_size[0]})\nfor several " if len(dom_size)==1 else "for several "
...
def compare_swd_decoupled(set_path, steps=[[51000,102000,147000]], add_name='_16384_16384', lat_dims=[512], chms=[2], bss=[16],
    ...
    suptitle = f"Comparison of decoupled SWD "
    suptitle += f"(domain = {dom_size[0]}x{dom_size[0]})\nfor several " if len(dom_size)==1 else "for several "
    save_path = set_path + f"Comparison_{mode}/" + (f"Dom_{dom_size[0]}/" if len(dom_size)==1 else "")
    if mode=='lat':
    ...
    if mode=='bs':
        ...
        suptitle += f"batch sizes"
    if mode=='chm':
        save_name = save_path + "compare_swd_decoup_chms_" + '_'.join(str(chm) for chm in chms) + f'_dom_{dom_size[0]}_steps_' + \
                '_'.join(str(step) for step in steps[0]) + f'_bs_{bs}_lat_{lat_dim}'
        suptitle += f"channel multipliers"
    if mode=='var':
    ...
    if mode=='noise':
        ...
        suptitle += f"noise configs"
    if mode=='domain':
        save_name = save_path + f"compare_swd_decoup_domains_{'_'.join(str(dom_s) for dom_s in dom_size)}_" +\
        '_'.join(str(var) for var in vars) + '_steps_' + '_'.join(str(step) for step in steps[0]) + f'_bs_{bs}_lat_{lat_dim}_chm_{chm}'
        suptitle += f"domain sizes"
    save_name += "_mean_pert" if mean_pert else "_no_mean" if no_mean else ""
...
def load_rgbs_concat(program, lat=512, bs=16, chm=2, lr=0.002, ckpt=147000, vars=['u','v','t2m'], 
                ...
                no_mean=False, mean_pert=False, samples_idx=[0],
                rgb_path="/home/poulainauzeaul/scratch-priam-sidev/Exp_StyleGAN/Finished_experiments/"):
    ...
    if not os.path.isfile(data_dir + name + ".npy"):
        print("Computing the concat ...")
    ...
    res = np.load(data_dir+name+".npy", mmap_mode='r')[samples_idx] # mmap_mode='r' -> le meilleur argument optionnel de tout numpy
...
def plot_samples(lat, bs, chm, ckpt, out_dir, lr=0.002, cmap_wind='viridis', dom_size=128, no_mean=False, mean_pert=False,
    ...
    if dom_size==128:
        ...
        real_sample = real_sample[:, :, 256:768, 256:768]
    elif dom_size==1024:
...
def plot_uni_v_multivar_samples(lat, bs, chm, ckpt, out_dir, lr=0.002, cmap_wind='viridis', dom_size=128, no_mean=False, 
            mean_pert=False, cmap_t='rainbow', varss=[['u','v','t2m'],['u','v']], use_noise=True, nb_samples=1, cherry_picked=False, 
            exp_path="/home/poulainauzeaul/scratch-priam-sidev/Exp_StyleGAN/Finished_experiments/",
            datapath="/home/poulainauzeaul/scratch-priam-sidev/Exp_StyleGAN/", 
            datapath_1024="/scratch/mrmn/brochetc/GAN_2D/datasets_full_indexing/",
            home_path="/home/poulainauzeaul/home-priam-sidev/"):
    ...
    if datapath_1024 is not None and dom_size in [512, 1024]:
        ...
        var_to_plot = varss[0]
        for vars in varss[1:]:
            var_to_plot = set(var_to_plot).intersection(set(vars))
        var_to_plot = sorted(list(var_to_plot), key=lambda var_f:var_dict[var_f])
        print(var_to_plot)

        if not mean_pert:
            Means = np.load(data_dir + means_fname)[[var_dict[var] for var in var_to_plot]].reshape(len(var_to_plot),1,1)
            Maxs = np.load(data_dir + maxs_fname)[[var_dict[var] for var in var_to_plot]].reshape(len(var_to_plot),1,1)
        # to be modif --> add possibility to use mean_pert=True
        else:
            var_idx = [var_dict[var] for var in vars]
            var_idx += [vi+8 for vi in var_idx]
            Means = np.load(data_dir + means_fname)[[var_id for var_id in var_idx]].reshape(len(var_idx),1,1)
    ...
    real_file = random.sample(glob(data_dir_0+f"_sample*"), 1)[0] 
    print("real: ", os.path.basename(real_file))
    real_sample = np.load(real_file)[[var_dict[var] for var in var_to_plot]]
    real_sample = real_sample.reshape(-1, *real_sample.shape)
    ...
    fsamp_dirs = [exp_path +\
            ...
            ("_no_mean" if no_mean else "") +\
            ("_mean_pert" if mean_pert else "") +\
            "/Instance_1/samples/" for vars in varss]
    
    channels = [[vars.index(var) for var in var_to_plot] for vars in varss]

    if cherry_picked:
        idxs = [224,164,49,3]
        F_samples_lists = [[np.load(fsamp_dir+f"_Fsample_{ckpt}_42.npy")[idxs[0]], np.load(fsamp_dir+f"_Fsample_{ckpt}_115.npy")[idxs[1]], 
                np.load(fsamp_dir+f"_Fsample_{ckpt}_54.npy")[idxs[2]], np.load(fsamp_dir+f"_Fsample_{ckpt}_186.npy")[idxs[3]]] \
                for fsamp_dir in fsamp_dirs]
        files = [fsamp_dirs[0]+f"_Fsample_{ckpt}_42.npy", fsamp_dirs[0]+f"_Fsample_{ckpt}_115.npy", \
                 fsamp_dirs[0]+f"_Fsample_{ckpt}_54.npy", fsamp_dirs[0]+f"_Fsample_{ckpt}_186.npy"
                ][:nb_samples]
        F_samples = np.array([[F_samp[channels[i],:,:]*Stds+Means for F_samp in F_samples_list[:nb_samples]] \
                              for i, F_samples_list in enumerate(F_samples_lists)])
    else:
        F_samples_lists = [glob(fsamp_dir+f"_Fsample_{ckpt}_*") for fsamp_dir in fsamp_dirs]
        files = [random.sample(F_samples_list,1)[0] for F_samples_list in F_samples_lists]
        F_samples = [np.load(file) for file in files]
        idxs = [np.random.randint(low=0, high=F_sample.shape[0],size=nb_samples) for F_sample in F_samples]

        
        #print(channels, idxs)
        #print([F_samp.shape for F_samp in F_samples])
        F_samples = [F_sample[idxs[i]][:,channels[i],:,:]*Stds+Means for i, F_sample in enumerate(F_samples)]
        #print([F_samp.shape for F_samp in F_samples])
        if dom_size==1024:
            F_samples = [F_sample[:, :, 153:870, :] for F_sample in F_samples]
        
        # to be modified --> possibility to have mean_pert!=False
        if mean_pert:
            F_samp_tmp = np.zeros((nb_samples, len(vars), dom_size, dom_size))
            for i in range(F_samp_tmp.shape[1]):
                F_samp_tmp[:, i, :, :] = F_samples[:, i, :, :] + F_samples[:, i+len(vars), :, :]
            F_samples = F_samp_tmp
    print("indexes: ", idxs,"\nfake: ", [os.path.basename(file) for file in files])
    
    pic_name = f"uni_v_multi_{nb_samples}_" + ("rdm" if not cherry_picked else f"cherry") +f"_sample_dom_{dom_size}_lat_{lat}_bs_{bs}_"+\
                f"chm_{chm}_lr_{lr}_ckpt_{ckpt}_{'|'.join('_'.join(var for var in vars) for vars in varss)}_noise_{use_noise}"
    pic_name += "_no_mean.png" if no_mean else "_mean_pert.png" if mean_pert else ".png"
    col_titles = ["AROME-EPS"] +\
    flatten([[f"GAN({multiline_label(','.join(var for var in vars))})" for i in range(nb_samples)] for vars in varss])

    suptitle = f"Randomly generated samples"
    data = np.concatenate((real_sample, *[F_sample for F_sample in F_samples]), axis=0)
    
    zone = Zones[dom_size]
    can = art.canvasHolder(zone, nb_lon=dom_size, nb_lat=np.min([dom_size, 717]), fpath=home_path)

    var_names = [(var, var_dict_unit[var]) for var in var_to_plot]
    can.plot_abs_error_sev_cbar(data=data, var_names=var_names, suptitle=suptitle,
                        plot_dir=out_dir, pic_name=pic_name, col_titles=col_titles, cmap_wind=cmap_wind, cmap_t=cmap_t)


def plot_rgb(lat, bs, chm, ckpt, out_dir, lr=0.002, cmap_wind='viridis', dom_size=128, no_mean=False, 
     mean_pert=False, cmap_t='rainbow', vars=['u','v','t2m'], rgb_level=3, use_noise=True, resolutions=[128], N=65536,
     rgb_path="/home/poulainauzeaul/scratch-priam-sidev/Exp_StyleGAN/Finished_experiments/", 
     datapath="/home/poulainauzeaul/scratch-priam-sidev/Exp_StyleGAN/",
     home_path="/home/poulainauzeaul/home-priam-sidev/"):
    ...
    res_to_plot = [load_rgbs_concat(program=program, lat=lat, bs=bs, chm=chm, lr=lr, ckpt=ckpt, vars=vars, rgb_level=rgb_level, 
                    use_noise=use_noise, dom_size=dom_size, res=res, no_mean=no_mean, mean_pert=mean_pert, 
                    N=N, rgb_path=rgb_path, samples_idx=index)[0] for res in resolutions]
    ...
    pic_name = f"plot_rdm_rgb_dom_{dom_size}_level_{rgb_level}_lat_{lat}_bs_{bs}_chm_{chm}_lr_{lr}_ckpt_{ckpt}_{up_mode}_upsamp"\
                + f"_{'_'.join(var for var in vars)}_noise_{use_noise}_{nb_res}_res"
    pic_name += "_no_mean.png" if no_mean else "_mean_pert.png" if mean_pert else ".png"

    #col_titles = [(2**(i+2),2**(i+2)) for i in range(nb_res-nb_res_to_plot, nb_res)]
    col_titles = [str(res) for res in resolutions]

    suptitle = f"Random input after each RGB layer (upsampled to {dom_size}x{dom_size})"
    
    
    data = np.zeros((nb_res, len(vars), dom_size, dom_size))
    Upsampler = torch.nn.Upsample(size=dom_size, mode=up_mode)

    """for i, key in enumerate(list(res_to_plot.keys())[-nb_res:]):
        ...
        data[i] = img_up"""
    for i, im in enumerate(res_to_plot):
...
"""def plot_inversion_error_comparison_lats(lat_dims, bs, chm, out_dir, date_index=3, 
    ...
    Means = np.load(data_dir + 'mean_with_8_var.npy')[1:4].reshape(3,1,1)
    Maxs = np.load(data_dir + 'max_with_8_var.npy')[1:4].reshape(3,1,1)        
    Stds = (1.0/0.95) * Maxs
...
##
def load_fake_real(data_dir, file_loc, lat_dim, bs, chm, date_index, lead_time, perturb=None, mode='lat', loss='mse'):
