...
from criteria import id_loss, w_norm, moco_loss, scattering_loss
from criteria.SWD_loss import SwdLoss
from configs import data_configs
from datasets.arome_dataset import AromeDataset
...
class Coach:
    def __init__(self, opts):
        self.opts = opts
        self.global_step = self.opts.resume_step + 1 if self.opts.resume_step!=0 else 0
        ...
        np.save(os.path.join(self.opts.exp_dir, 'avg_sample.pth'), self.avg_sample.cpu().numpy())
    ...
    def perform_train_iteration_on_batch(self, x, y):
        ...
        for iter in range(self.opts.n_iters_per_batch):
            if iter == 0:
                avg_image_for_batch = self.avg_sample.unsqueeze(0).repeat(x.shape[0], 1, 1, 1)
    ...
    def validate(self):
        ...
        if self.global_step // self.opts.val_interval==1 :
    ...
    def configure_optimizers(self):
        ...
        if self.opts.optim_name == 'adam':
            optimizer = torch.optim.Adam(params, lr=self.opts.learning_rate, weight_decay = 0.0005)
        else:
    ...
    def configure_datasets(self):
        ...
        transforms_dict = dataset_args['transforms'](self.opts, path).get_transforms()
        train_dataset = AromeDataset('IS_method_labels_split.csv',[1,2,3],
                                     [78,206,55,183],
        ...
        transforms_dict = dataset_args['transforms'](self.opts, path).get_transforms()
        test_dataset = AromeDataset('IS_method_labels_split.csv',[1,2,3],
                                     [78,206,55,183],
    ...
    def calc_loss(self, x, y, y_hat, latent, option ='train'):
        ...
        if self.opts.scat_lambda > 0 :  # Scattering loss
            loss_scat = self.scat_loss(y_hat, y)
            loss_dict['scat_loss'] = float(loss_scat)
            loss += loss_scat * self.opts.scat_lambda
        if self.opts.swd_lambda > 0 :  # Scattering loss
            loss_swd = self.swd_loss.End2End(y_hat, y)
            loss_dict['swd_loss'] = float(loss_swd)
            loss += loss_swd * self.opts.swd_lambda
    ...
    def log_images(self, name, sample_data, subscript=None, log_latest=False):
        ...
        fig = common.vis_samples(sample_data, self.opts.n_vars)
