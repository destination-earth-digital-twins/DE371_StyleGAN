...
from configs.paths_config_2 import model_paths
...
class pSp(nn.Module):
    ...
    def __get_encoder_checkpoint(self):
        if "ffhq" in self.opts.dataset_type:
            ...
            if self.opts.input_nc != 3:
                shape = encoder_ckpt['module.encoder_k.conv1.weight'].shape
                altered_input_layer = torch.randn(shape[0], self.opts.input_nc, shape[2], shape[3], dtype=torch.float32)
                altered_input_layer[:, :3, :, :] = encoder_ckpt['module.encoder_k.conv1.weight']
                encoder_ckpt['module.encoder_k.conv1.weight'] = altered_input_layer
            mapped_encoder_ckpt = dict(encoder_ckpt)
            
            for p in encoder_ckpt.keys():
                if 'encoder_q' in p :
                    mapped_encoder_ckpt.pop(p)
            
            
            print('names swapping, step 1')
            ...
            for p,v in encoder_ckpt.items():
                try :
                    ...
                    pass
            print('names swapping, step 2')  
            
            for p,v in encoder_ckpt.items():
                try :
                    mapped_encoder_ckpt[p.replace('module.encoder_k', '')] = v
                    mapped_encoder_ckpt.pop(p)
                except KeyError :
                        pass
