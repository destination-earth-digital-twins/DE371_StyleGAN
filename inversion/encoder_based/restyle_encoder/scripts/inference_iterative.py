...
from utils.common import numpyfy
...
def run():
    ...
    transforms_dict = dataset_args['transforms'](opts, path = opts.data_path+'/').get_transforms()
    dataset = AromeDataset('IS_method_labels_split.csv',[1,2,3],
                                     [78,206,55,183],
    ...
    if opts.n_samples is None:
        opts.n_samples = len(dataset)
    ...
    input_idx = '_sample40806'
    
    sample,_ = dataset.__getitem__(input_idx, mode='sample')
    
    input_batch = sample.unsqueeze(0)
    ...
    for i in range(input_batch.shape[0]):
        results = [numpyfy(result_batch[i][iter_idx]) for iter_idx in range(opts.n_iters_per_batch)]
        im_path = '_Fsample_40806.npy'

        # save step-by-step results side-by-side
        for idx, result in enumerate(results):
            save_dir = os.path.join(out_path_results, str(idx))
            os.makedirs(save_dir, exist_ok=True)
            np.save(os.path.join(save_dir, os.path.basename(im_path)), result)
