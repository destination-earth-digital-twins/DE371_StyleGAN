...
class PatchVGGPerceptualLoss(torch.nn.Module):
    def __init__(self, resize=False, state_dict_path='', init_layer=False, vgg_single_channel_input=True, split_factor=2):
        super(PatchVGGPerceptualLoss, self).__init__()
        blocks = []
    ...
    def forward_given_features(self, target_img, feature_layers=[0,1,2,3], features_input_img=None, style_layers=[], styles_input_img=None, alpha_feature=1.0, alpha_style=0.01):
        ...
        for j, y in enumerate(self.splitter(y)):
            for i, block in enumerate(self.blocks):
                y = block(y)
                if i in feature_layers:
                    x = features_input_img[i+j] # TODO : do not work for some reason that must be investiguated
    ...
    def splitter(self, img):
        r''' Split an input image in equal patches '''
        b, c, h, w = img.size()
        assert self.split_factor % 2 == 0
        splitted_imgs = []
        splits_id = list(range(0, h+1, h//self.split_factor))
        for i in range(len(splits_id)-1):
            for j in range(len(splits_id)-1):
                splitted_imgs.append(img[:,:, splits_id[i]:splits_id[i+1], splits_id[j]:splits_id[j+1]])
    ...
    def forward(self, input_img, target_img, feature_layers=[0,1,2,3], style_layers=[], alpha_feature=1.0, alpha_style=0.01):
        ...
        for x, y in zip(self.splitter(x), self.splitter(y)):
            for i, block in enumerate(self.blocks):
                x = block(x)
                y = block(y)
                if i in feature_layers:
