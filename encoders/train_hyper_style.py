...
from encoders.options.hyperStyle_train_options import HyperStyleTrainOptions, createNamesFromLosses
from encoders.training.coach_hyperstyle import Coach


def main(namesFromLosses=False):
    
    config = HyperStyleTrainOptions().parse()
    
    if namesFromLosses : 
        config.exp_dir = config.exp_dir + 'HyperStyle_training/' +  createNamesFromLosses(config)
