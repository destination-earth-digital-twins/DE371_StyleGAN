...
from encoders.utils.inference_utils import run_on_batch
...
class Predictor(BasePredictor):
    ...
    def run_default_encoding(
        ...
        config = ckpt["config"]
        config["checkpoint_path"] = self.model_paths[encoding_type]
        config = Namespace(**config)
        net = e4e(config) if encoding_type == "horses" else pSp(config)
        net.eval()
        net.cuda()
        print("Done!")

        # define some arguments
        config.n_iters_per_batch = num_iterations
        ...
        with torch.no_grad():
            ...
            result_batch, result_latents = run_on_batch(
                transformed_image.unsqueeze(0).cuda(), net, config, avg_image
            )
        ...
        resize_amount = (
            ...
            else (config.output_size, config.output_size)
        )
        res = self.get_final_output(
            result_batch, resize_amount, display_intermediate_results, config
        )
    ...
    def run_toonify_bootstrapping(
        ...
        config = ckpt["config"]
        config["checkpoint_path"] = self.model_paths["faces"]
        config = Namespace(**config)
        net_ffhq = pSp(config)
        net_ffhq.eval()
        net_ffhq.cuda()
        ...
        config = ckpt["config"]
        config["checkpoint_path"] = self.model_paths["toonify"]
        config = Namespace(**config)
        net_toonify = pSp(config)
        net_toonify.eval()
        net_toonify.cuda()
        print("Done!")
    ...
    def get_final_output(
        ...
        if display_intermediate_results:
            result_images = [
                ...
                for iter_idx in range(config.n_iters_per_batch)
            ]
        else:
